...
n = smal_n % the smallest odd size of the filter
...
im_filt = imfilter(im, fbox, 'replicate', 'conv'); % the filtered image
...
% Your answer here:
%
% The smallest odd sized kernel that we can use for a 2x2 object is a 7x7 kernel according to 
% the formula we derived above. If a small 2x2 object is close to another, the kernel will contain 
% some values of that other object as well. That means that the average value in the filter 
% kernel will be bigger than Q/4 and it will determine that the object is larger than it actual is 
% and therefore not deleting it.
%