...
RGB_Image = im2double(RGB_Image); % convert both of the input images to doubles
...
Lower = IR_Image(round(y),round(x)) - LevelRange/2; % The lowest intensity value in the selected range
Higher = IR_Image(round(y),round(x)) + LevelRange/2; % The highest intensity value in the selected range
% x and y values are flipped by the function ginput, so we flip the coordinates
% round the values so that they can be used as array index
...
Mask = (IR_Image>Lower & IR_Image<Higher); % sets the mask values to 1 according to min and max, else 0
% Use the Mask to mask out the areas within ths selected IR-range in the RGB-images
% (for all 3 color channels)

RGB_Range = RGB_Image.*Mask; % elementwise multiplication of the mask and image (will automatically do for all channels)
...
IR_Select(round(y)-5:round(y)+5, round(x)-5:round(x)+5, 1) = 1; % Create a red square 10x10 pixels around the chosen pixel 
IR_Select(round(y)-5:round(y)+5, round(x)-5:round(x)+5, 2) = 0;
IR_Select(round(y)-5:round(y)+5, round(x)-5:round(x)+5, 3) = 0;
