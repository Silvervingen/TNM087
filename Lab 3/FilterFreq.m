...
fp= zeros(P,Q) % a matrix of dubbel of the size of the image
fp(1:M, 1:N) = f; % assigns the values of the image to the matrix which makes a zero padded image 
...
F = fftshift(fft2(fp)); % Shifting the fourier transform
...
olpf= real(ifft2(ifftshift(OLP))); % Shifting back the image to the left corner

%% Find the lowpass filtered image
% Extract the final lowpass filtered image from the padded lowpass filtered
% image

olp= olpf(1:M, 1:N); % The final image of filtered image

%% Find the final highpass filtered image
% Read the document for this task

ohp= f - olp; % Gives the image when filter value is taken out of the orginal image.
