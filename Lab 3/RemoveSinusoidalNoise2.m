function out=RemoveSinusoidalNoise2(in,D0)
%
...
M = floor(P/2 + 1); % center is at +1  because matlab index.
N = floor(Q/2 + 1);
F2((M-2:M+2),(N-2:N+2)) = 0;

maximum = max(F2(:));
[a, b] = find(F2==maximum);

r= a(1); % the row number of one the two dominant peaks
c= b(1); % the column number of the same peak as above
...
vk = c - N; % uk and vk are the positions of the peaks relative the center of the spectrum
uk = r - M; %
...
out = real(ifft2(ifftshift(F_filtered))); % the final output image, where the most dominant sinusoidal noise is eliminated
